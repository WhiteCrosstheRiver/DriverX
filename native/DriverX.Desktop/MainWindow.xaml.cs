...
public partial class MainWindow : Window
{
 ...
 public MainWindow(){InitializeComponent();Microsoft.Win32.SystemEvents.UserPreferenceChanged+=SystemPreferenceChanged;trayIcon=CreateTrayIcon();LoadTheme();LoadProfiles();ConnectionList.ItemsSource=profiles;RenderProtocols();UpdateStatus();}
 ...
 void RenderProtocols(){ProtocolCards.Children.Clear();foreach(var p in new[]{("SFTP","SSH 安全文件传输"),("WebDAV","标准网络文件访问"),("FTP","传统文件传输"),("SMB","Windows 网络共享"),("S3","对象存储"),("OneDrive","Microsoft 云盘"),("Google Drive","Google 云端硬盘"),("Dropbox","Dropbox 云存储")}){var b=new Border{Width=250,Height=100,Margin=new(0,0,16,16),Padding=new(18),CornerRadius=new(12),Background=(System.Windows.Media.Brush)FindResource("CardBackground"),BorderBrush=(System.Windows.Media.Brush)FindResource("BorderBrush"),BorderThickness=new(1)};b.Child=new StackPanel{Children={new TextBlock{Text=p.Item1,FontSize=16,FontWeight=FontWeights.SemiBold},new TextBlock{Text=p.Item2,Margin=new(0,7,0,0),Foreground=(System.Windows.Media.Brush)FindResource("TextSecondary")}}};b.SetResourceReference(Border.BackgroundProperty,"CardBackground");b.SetResourceReference(Border.BorderBrushProperty,"BorderBrush");var texts=((StackPanel)b.Child).Children.Cast<TextBlock>().ToArray();texts[0].SetResourceReference(TextBlock.FontSizeProperty,"CardTitleSize");texts[1].SetResourceReference(TextBlock.ForegroundProperty,"TextSecondary");ProtocolCards.Children.Add(b);}}
